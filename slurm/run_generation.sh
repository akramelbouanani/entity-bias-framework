...
#SBATCH --job-name=sentence-generation
...
mkdir -p "${REPO_ROOT}/slurm/logs/vllm"

for MODEL_NAME in "${MODELS[@]}"; do
    MODEL_PATH="${MODEL_PATH_COMMON}/${MODEL_NAME}"
    SERVER_LOG="${REPO_ROOT}/slurm/logs/vllm/${SLURM_JOB_ID:-local}-${MODEL_NAME}-generation.log"
    ...
    for TASK in "${TASKS[@]}"; do
        echo "Generating ${TASK} with ${MODEL_NAME}..."
        python3 -m bias_audit.cli generate "${TASK}" \
            --port "${PORT}" \
            --languages "${LANGUAGES[@]}" \
...
echo "Sentence generation completed."
