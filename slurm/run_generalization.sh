...
#SBATCH --job-name=entity-audit

set -euo pipefail

# Slurm executes a copied script from /var/spool/slurm, so use the submission
# directory. This accepts submission from either the repository root or slurm/.
...
mkdir -p "${REPO_ROOT}/outputs/generalization" "${REPO_ROOT}/slurm/logs/vllm"
...
for MODEL_NAME in "${MODELS[@]}"; do
    ...
    python3 "${REPO_ROOT}/src/run_generalization.py" \
        --tasks "${TASKS[@]}" \
        --few-shot "${FEW_SHOT[@]}" \
        --numerical "${NUMERICAL[@]}" \
        --languages "${LANGUAGES[@]}" \
