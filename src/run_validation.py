#!/usr/bin/env python3
"""Run configured synthetic-to-real validation experiments."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--variants", nargs="+", choices=["original", "synthetic"], default=["original", "synthetic"])
    parser.add_argument("--config-dir", type=Path, default=DEFAULT_CONFIG_DIR)
    parser.add_argument("--output-dir", type=Path, default=PROJECT_ROOT / "outputs" / "validation")
...
def main() -> None:
    ...
    for task_id in args.tasks:
        ...
        if task.suite != "validation":
            raise ConfigError(f"Task '{task_id}' belongs to suite '{task.suite}', not 'validation'")
        entity_set = registry.entity_set(task.entity_set)
        for language in args.languages:
            for variant in args.variants:
                frame = expand_task(task, entity_set, language=language, variant=variant)
                for few_shot in args.few_shot:
